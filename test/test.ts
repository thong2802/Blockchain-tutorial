...
describe("Vault", function() {
    ...
    it("Should not deposit, Insufficient accounts balance", async () => {
        await token.transfer(alice.address,parseEther(1 * 10**6));
        await token.connect(alice).approve(vault.address,token.balanceOf(alice.address));
        await expect (vault.connect(alice).deposit(parseEther(2 * 10**6))).revertedWith('Insufficient accounts balance');
    });
    ...
    it('Should not withdraw, caller is not a withdraw', async () => {
        ...
        // alice deposit into the alice address
        await token.transfer(alice.address, parseEther(1 * 10**6));
        await token.connect(alice).approve(vault.address, token.balanceOf(alice.address));
        await vault.connect(alice).deposit(parseEther(500*10**3));

        // carol withdraw into the alice address.
        await expect(vault.connect(carol).withdraw(parseEther(2*10**3), alice.address)).revertedWith("caller is not a withdraw");
    });
